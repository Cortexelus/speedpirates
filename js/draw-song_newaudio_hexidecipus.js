function Song(id, mp3, song_title, artist, segments, palette){
    ...
    this.num_channels = 2
   	...
   	this.just_first_segment = true
   	this.jump_to_point = false
   	this.random_segment_from_chunks = true
	...
	function PaletteMapper(palette_mapper_id, palette_mapper_highlight_id, segments, palette){
		...
		this.draw = function (){
			...
			for (p=0; p<totalSegments; p+=1){ 
				...
				context.fillStyle = alphaColor(color, alpha); // straight color
	...
	this.play_list = function(play_segment_list){
		...
		if(this.sound_loaded){
			...
			if(this.random_segment_from_chunks){
				rand_seg = Math.floor(Math.random()*play_segment_list.length);
				segment_id = play_segment_list[rand_seg].segment_id
				segment = this.segments[segment_id]
				this.play(then, segment.start, segment.duration, segment_id, this.pauseVideo, 1 )
			}else if(this.just_first_segment){
				console.log("sdf")
				segment_id = play_segment_list[s].segment_id
				segment = this.segments[segment_id]
				this.play(then, segment.start, segment.duration, segment_id, this.pauseVideo, 1 )
			}else if(this.jump_to_point){
				segment_id = play_segment_list[s].segment_id
				segment = this.segments[segment_id]
				this.play(then, segment.start, 30, segment_id, this.pauseVideo, 1 )
			}else{
    ...
    this.sortable_segment_list = function(id){
    	elem = $(id);
    	//console.log(elem)
    	elem.append("<li class='ui-state-default'></li>")
    	elem.append("<li class='ui-state-default'></li>")
    }
