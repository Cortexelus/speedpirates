function Song(id, mp3s, song_title, artist, segments, palette){
    this.song_id = id
    this.mp3s = mp3s
    ...
    this.num_channels = 2
   	...
   	this.have_video = true // true if we're remixing a video
    ...
    for(var i=0;i<this.mp3s.length;i++){
    	var sound = new load_sound(this.mp3s[i], function(){
	...
	this.play = function(sound, w,s,d, callback, gain){
		sound.play(w,s,d, callback,gain)
	};
	...
	this.play_list = function(sound, play_segment_list){
		...
		if(this.sound_loaded){
			...
			for(var loops=0;loops<100;loops++){
				for(var s=0;s<play_segment_list.length;s++){
					...
					this.play(sound, then, 0, 0.0001,segment_id,this.reactToSegment,0)
					if(true){//step_duration <= segment.duration){
					// always use step_duration. 
						this.play(sound, then, segment.start, step_duration, segment_id, null, 1 )
					}else{
						this.play(sound, then, segment.start, segment.duration, segment_id, this.pauseVideo, 1 )
					}
